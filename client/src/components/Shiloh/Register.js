...
import { SHILOH_SIGNUP } from '../../_actions/types';
import { setAlert } from '../../_actions/alertActions';

const Register = ({ closeModal, setAlert, registerForShiloh, newAttendee }) => {
  ...
  useEffect(() => {
    if(newAttendee !== null) closeModal();
  }, [newAttendee])
  ...
  const handleSubmit = e => {
    ...
    if (!accomodation || shiloh.availability.length < 1 ) {
      return setAlert("All fields are required", SHILOH_SIGNUP)
    }
  ...
  return ( 
    <Fragment>
      <form name="personal" onSubmit={handleSubmit} className="form">
          ...
          <button type="submit" className="btn btn-primary p-1"> Register </button>
      </form>
    </Fragment>
   );
}
...
export default connect(mapStateToProps, { registerForShiloh, setAlert})(Register);
