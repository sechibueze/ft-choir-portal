...
const { FRONTEND_URL, ADMIN_EMAIL, ALLOWED_STATUS} = require('../config/constants');
const { getPasswordRestLinkMessage, getOnboardingMessage } = require('../config/messages');

const getAllMembers = (req, res) => {
  Member.find({})
    .select('-password')
    .then(members => {
      let formatedMemberList = members.length > 0 ? members.map(member => ({
        _id: member._id,
...
const loginMember = (req, res) => {
  ...
  Member.findOne({ email }, (err, member) => {
    if (err) return res.status(500).json({ status: false, message: 'Server error:: Could not retrieve record', errors: err });
    ...
    bcrypt.compare(password, member.password, (err, isMatch) => {
      if (err) return res.status(500).json({ status: false, message: 'Server error:: Failed to compare password', errors: err });

      if (!isMatch) return res.status(401).json({ status: false, message: 'Account does not exist', errors: {} });
      ...
      jwt.sign(
        ...
        { expiresIn: 60 * 60 * 60 },
        (err, token) => {
          if (err) return res.status(500).json({ status: false, message: 'Server error:: Failed to generate token', errors: err });
...
const updateMemberData = (req, res) => {
  ...
  const {_id, key, access, firstname, middlename, lastname, email, status,password, phone} = req.body;
  if (!_id || !key || _id !== key) {
...
const toggleMembersAdminAuth = (req, res) => {
  ...
  Member
    ...
    .then(results => {
      ...
      const existingAdminsList = results.filter(member => member.auth.includes("admin") );
      const vacantAdminSlots = maxAllowabelAdmins - existingAdminsList.length;

      const fullMemberData = results.find(member => member.email === memberData.email)
      ...
      if ((vacantAdminSlots === 0 )) {
        if (isAlreadyAdmin) {
          fullMemberData.auth = fullMemberData.auth.filter(role => role !== "admin")   
        }
      }else  {
        // toggle
        if (isAlreadyAdmin) {
