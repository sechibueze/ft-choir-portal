...
const createPost = (req, res) => {
  ...
  Member.findOne({ _id: memberId}).select('-password')
    .then(member => {
...
const likePost = (req, res) => {
  ...
  Post.findOne({_id: postId})
    .then(post => {
      ...
      post.save(err => {
        if (err) return res.status(500).json({ status: false, error: 'Internal Server Error:: failed to update liked post' });
    ...
    .catch(err => {
      if (err) return res.status(500).json({ status: false, error: 'Internal Server Error:: failed to find post to like' });
    });
}
...
const unlikePost = (req, res) => {
  ...
  Post.findOne({ _id: postId })
    .then(post => {
      ...
      // Now unlike it
      post.likes = post.likes.filter(like => like.member.toString() !== memberId);
